...
export const Methodology = () => {
  ...
  return (
    <section id="metodologia" ref={ref as React.RefObject<HTMLElement>} className="py-12 sm:py-20 md:py-28">
      <div className="container px-4 sm:px-6">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <p
            className={`text-accent font-medium mb-2 text-center text-xs sm:text-sm transition-all duration-600 ${
          ...
          <h2
            className={`font-display text-lg sm:text-xl md:text-2xl text-center mb-6 sm:mb-10 transition-all duration-600 delay-100 ${
          ...
          {/* Principles - Compact cards */}
          <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-10">
            {principles.map((principle, index) => (
              <div
                key={index}
                className={`group flex items-start gap-3 glass-card p-3 sm:p-4 ${
                  isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
                }`}
                style={{ transitionDelay: isVisible ? `${(index + 2) * 50}ms` : "0ms" }}
              >
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-accent/10 flex items-center justify-center flex-shrink-0 mt-0.5 group-hover:bg-accent group-hover:scale-105 transition-all duration-300">
                  <Check className="w-3 h-3 text-accent group-hover:text-accent-foreground transition-colors" />
                </div>
          ...
          >
            <p className="text-center text-sm sm:text-base font-medium text-foreground/80 glass-card px-5 py-3 sm:px-6 sm:py-4">
              {language === "es"
