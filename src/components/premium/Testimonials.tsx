...
export const Testimonials = () => {
  ...
  return (
    <section id="testimonios" ref={ref as React.RefObject<HTMLElement>} className="py-24 md:py-32">
      <div className="container">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <p className={`text-accent font-medium mb-4 text-center transition-all duration-600 ${isVisible ? "opacity-100" : "opacity-0"}`}>
            {t("testimonials.title")}
          </p>
          <h2 className={`font-display text-3xl md:text-4xl text-center mb-12 transition-all duration-600 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4 mt-8">
              <button onClick={prev} className="w-12 h-12 rounded-full border border-border flex items-center justify-center hover:bg-muted transition-colors">
              ...
              <div className="flex gap-2">
                {testimonials.map((_, i) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${i === current ? "w-6 bg-accent" : "bg-border"}`}
                  />
                ))}
              </div>
              <button onClick={next} className="w-12 h-12 rounded-full border border-border flex items-center justify-center hover:bg-muted transition-colors">
